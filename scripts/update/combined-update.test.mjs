...
test("startup before commit restores source instead of adopting a dead coordinator", async () => {
...
test("interrupted commit may roll back only after native abort authorization", async () => {
...
test("combined rollback restores app selection before native service and remains blocked until healthy", async () => {
  const f = await fixture(selected);
...
test("interrupted service rollback preserves pending state and can replay source selection", async () => {
...
test("uncertain protected commit retries forward without rollback; failed restart retains commit", async () => {
  const f = await fixture([...selected, "DesktopReady", "Committing"]);
