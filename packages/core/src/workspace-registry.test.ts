...
const project: ProjectRecordV2 = {
  ...
  projectId: "project",
  label: "Project",
  unityProjectPath: path.resolve("/project"),
  repositoryRoot: path.resolve("/project"),
  unityRelativePath: ".",
  workspaceRoot: path.resolve("/workspaces"),
  storageCommand: path.resolve("/tools/storage.exe"),
...
const workspace = (index: number): WorkspaceRecordV2 => ({
  ...
  projectId: "project",
  name: `work-${index}`,
  workspacePath: path.resolve("/workspaces", String(index)),
  storageWorkspaceId: `storage-${index}`,
  storageWorkspacePath: path.resolve("/storage", String(index)),
  mountPath: path.resolve("/storage", String(index), "Library"),
