...
The automated Windows vertical slice is green: repository quality gates, architecture rules, strict typechecks, both esbuild bundles, 236 Vitest cases, a real packaged-runtime PTY round trip, the Git for Windows bundled Vim TUI smoke, and the official VS Code Extension Host smoke test passed. Eleven cases in the required matrix are PASS; the visual/IME/Neovim case is BLOCKED and is deliberately not counted as a pass.
...
| `corepack pnpm verify`                                                                                      | PASS; secret scan, production license audit, Prettier, ESLint, strict typecheck, TypeScript/esbuild build, 56 files/236 tests, and dependency-cruiser (141 modules/188 dependencies). |
...
## Live incremental terminal rendering release blocker (2026-08-01)

### Observed failure and boundary

Manual QA found that a live `cmd.exe /D /K` Run showed its initial prompt but did not render later commands or results in xterm. The same markers appeared in the PTY log. This proved that terminal input, Runtime dispatch, ConPTY, the child process, and file logging were healthy while the incremental Runtime-output-to-Webview rendering path was not.
...
The Activity Bar now retains the Session Tree only. Selecting a Session or invoking **Honey Bee: Open Console** opens a wide editor-area Console. A single-line Run Bar contains Session/Run context, selector, connection/status, Start/Interrupt/Stop, Log, and Compose Prompt actions. Raw Terminal owns the remaining space. Prompt Composer starts collapsed and opens only from its action or `Ctrl+Alt+P`. Selecting a new live Run focuses its terminal; archived Runs retain the existing read-only policy.
...
2. Confirm the Honey Bee activity-bar icon and Sessions tree render, selecting a Session opens the editor-area Console, and the one-line Run Bar plus Start/Interrupt/Stop/Compose Prompt controls render without clipping at 100%, 125%, 150%, and approximately 200% zoom.
...
- The replacement VSIX is an unsigned local QA artifact only. It is not a tag, GitHub Release, Marketplace upload, merge approval, or substitute for installed-package manual validation.
